...
function recordTimeEntries(start, end, sheetName) {
...
// 取得した開始日 (start) と終了日 (end) の日付を整形。日付オブジェクト (Date クラスのインスタンス) を作成し、それぞれの日付の時間部分を設定。
  ...
  // for ループの開始を示します。i という変数を 0 から始め、events 配列の長さ（イベントの総数）未満の条件を満たす限りループを続けます。各ループで i の値が 1 ずつ増加
...
var tomorrow = new Date(today.getFullYear(), today.getMonth(), today.getDate() + 1); //today から1日後
  ...
  recordTimeEntries(startDate, yesterday, gSheetNameResult);
